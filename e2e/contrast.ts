 ...
 *    an unknown backdrop, or a `color-mix()`. This page is built out of them:
 *    every `.hex-output` and every `.state-value` — which is where nearly all of
 *    the generated bytes on this page are shown — sits on `rgba(0,0,0,0.4)`
 *    (`0.06` in light) over whatever panel is behind it, and `.uf-func`, the
 *    HMAC box at the centre of the state-update diagram, is
 *    `color-mix(in srgb, var(--blue-info) 12%, transparent)`. The hero aside is
 *    a `color-mix(in oklab, var(--green-clean) 6%, transparent)`, and the shared
 *    top bar's `--cl-ink` is another. axe files all of them under `incomplete`,
 *    which means a violations-only gate measured the contrast of almost nothing
 *    this lab actually generates.
 ...
 *    at `.85`, `.btn:disabled` at `.5`, and — the one that matters — `<body>`
 *    itself, which runs `crt-flicker 4s infinite` and holds the whole document
 *    between `opacity: 1` and `0.97` for as long as the page is open. The gate
 *    asks for reduced motion, which cancels that animation and pins the document
 *    at 1; without it every ratio on the page would drift by a per-scan amount
 *    nobody chose.
 *
 ...
 *    boxes are `.hex-output` (`max-height: 8rem; overflow-y: auto`, which every
 *    generated byte stream lands in and overflows as soon as enough bytes are
 *    asked for), `.state-value` and `.uf-hex` (both `overflow: hidden` with
 *    `text-overflow: ellipsis` around a full hex state), and, at phone width
 *    only, `.cmp-table` (`display: block; overflow-x: auto`). Content scrolled
 *    or ellipsed past those is not dimmed or partly drawn — it is absent from
 ...
 *    draws its figures — the flow diagram, the update-flow box — in HTML and
 *    CSS grid rather than SVG, so the sibling walk finds nothing to do. It is
 ...
 * rule. This page has one that carries meaning, measured by hand from real
 * screenshot pixels instead: `.gloss > summary::before { content: '?' }`. That
 * glyph, in a 1px circle, is the entire affordance marking a glossary term as
 * expandable — the rule that draws it also sets `list-style: none` and hides the
 * `::-webkit-details-marker`, so the UA's own disclosure triangle is gone and
 * this is what replaced it. Painted `--blue-info` on the `--bg-secondary` fill
 * of `.gloss`: 5.56:1 dark and 6.40:1 light, clearing both the 4.5:1 an 11.2px
 * glyph needs and the 3:1 SC 1.4.11 asks of its ring.
 *
 * SECOND, and more unusually: `body::after` is a FIXED, FULL-VIEWPORT SCANLINE
 * OVERLAY — `repeating-linear-gradient` of `rgba(0,0,0,0.03)` stripes at
 * `z-index: 9999` — painted OVER every pixel on the page. It is not an ancestor
 * of anything, so this walk (which composites ancestors) does not see it, and
 * neither does axe. It therefore darkens the real rendering by up to 3% on half
 * the scanlines, below everything measured here.
 *
 * That is a genuine gap and it is bounded rather than waved away: the same
 * reduced-motion block the gate asserts sets `body::after { display: none }`, so
 * in the configuration this gate measures the overlay is not painted at all, and
 * every ratio reported is exact for that rendering. For a reader who has NOT
 * asked for reduced motion, the true ratios are within about 1.5% of these
 * (3% alpha over half the rows), which no measurement on this page is close
 * enough to the floor to care about — the tightest is 4.85:1.
 */
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * that is every `.hex-output` (`overflow-y: auto` under an 8rem cap),
     * `.state-value` and `.uf-hex` (`overflow: hidden` with an ellipsis), and
     * `.cmp-table` at phone width. `.sr-only` is handled separately below.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // five links carry a `<span class="sr-only"> (opens in new tab)</span>`,
      // while `#sr-announcer` is an `aria-live="assertive"` region that is
      // `.sr-only` and whose text changes on every run. Every one of those has
      // a rect and passes `checkVisibility()` while painting exactly zero
      // pixels; measuring them reports ratios for ink that was never laid down,
      // and `#sr-announcer` would report a fresh one on every scan. It is
     ...
     * the emoji that opens each `.panel-header` (🔬 🔒 ⚙️ 📤 ⚖️ 📊 📐 🏗️ ⚠️ 🛡️ 🔄
     * 🕵️ 🧾), and the `→`/`↩` `.flow-arrow`s between the nodes of the data-flow
     * diagram. In every case the words that carry the meaning sit immediately
     * beside the glyph, in the same ink on the same surface, so measuring the
     * heading measures the glyph too.
     *
     * Nothing on this page hides a VALUE. No generated byte, no state, no
     * verdict and no test result is inside an `aria-hidden` subtree — which was
     ...
     *
     * This lab has no SVG of its own — every figure it draws is HTML and CSS
     * grid. The guard stays because the usual way the mistake arrives is exactly
